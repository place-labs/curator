...
describe Curator::Filter::Rules do
  ...
  describe "#pass?" do
    it "when event passes through all inclusive rules it returns true" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(true)
    end

    it "when event cant pass through evt rule it returns false" do
      event = SpecHelper.get_event({"evt": "invalid-evt", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through uts rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617004, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through org rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "fb", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through bld rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-2", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through lvl rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-2", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through loc rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-2", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through src rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jjj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through val rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.1, "mod": "mod-1", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through mod rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-2", "ref": "user@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end

    it "when event cant pass through ref rule it returns false" do
      event = SpecHelper.get_event({"evt": "presence", "uts": 1580276617006, "org": "apple", "bld": "bld-1", "lvl": "lvl-1", "loc": "loc-1", "src": "jj@example.com", "val": 1.0, "mod": "mod-1", "ref": "user2@example.com"})
      RulesHelper.include_rules.pass?(event).should eq(false)
    end
